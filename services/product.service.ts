...
export class ProductService {
  ...
  public static readonly getLatestProducts = async () => {
    return await Product.find({}, { sort: { created_at: 'DESC' }, limit: 10 }).then((products) => {
      return products.rows;
    });
  };
  private static readonly safeFind = async <T>(model: any, query: object): Promise<T[]> => {
    const result = await model.find(query);
    return Array.isArray(result) ? result : result.rows || [];
  };
  public static readonly getUserRecommendedProducts = async (user_id: string): Promise<IProduct[]> => {
    ...
    for (const query of searchHistoryQueries) {
      ...
      const foundProducts = (await this.safeFind<IProduct>(Product, { is_active: true }))
        .filter((product:IProduct) =>
          product.name?.toLowerCase().includes(queryLower)
          || product.description?.toLowerCase().includes(queryLower)
          || Object.values(product.meta_data || {}).some(val => val.toLowerCase().includes(queryLower))
        );
    ...
    const uniqueProducts: IProduct[] = Array.from(new Set(allProducts.map(p => p.id)))
