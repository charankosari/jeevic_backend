...
export const createOrder = async (orderData: any) => {
  ...
  const res = await axios.post(`${BASE_URL}/orders/create/adhoc`, orderData, { headers });
  return res.data;
};
...
export const updateOrder = async ( updatedData: any) => {
  const headers = await getAuthHeaders();
  const res = await axios.put(`${BASE_URL}/orders/update/adhoc`, updatedData, { headers });
  return res.data;
};
...
export const trackByShipmentId = async (shipmentId: string) => {
  const headers = await getAuthHeaders();
  const res = await axios.get(`${BASE_URL}/courier/track/shipment/${shipmentId}`, { headers });
  return res.data;
};
