...
export class OrderController {
    ...
    public static readonly createOrder = async (ctx: Context) => {
        try {
            ...
            for (const item of cartItems) {
                ...
                if (product) {
                    const price = await SaleService.getDiscountedPrice(product.id, product.price);
            ...
            if (coupon_code) {
                ...
                if (coupon && coupon.expires_on > new Date() && coupon.no_of_uses > 0) {
                    coupon_data = {
                        coupon_id: coupon.id,
                        discount_type: coupon.meta_data.discount_type as 'fixed' | 'percentage',
                        discount_value: coupon.meta_data.discount_value as unknown as number,
                    };
                    ...
                    // Add Discount Amount
                    discount_amount += coupon_data.discount_type === 'fixed' ? coupon_data.discount_value! : 0;
                }
            }
            ...
            // Get Gift Card ID
            if (gift_card_code) {
                const gift_card = await GiftCardService.getGiftCardByCode(gift_card_code);

                if (gift_card && gift_card.expires_on > new Date() && !gift_card.is_redeemed) {
                    gift_card_data = {
            ...
            const total_amount = order_value + tax_amount + shipping_amount - discount_amount;
    ...
    private static async handleRazorpayEvent(
		...
		switch (webhook_payload.event) {
			case 'payment.authorized': {
				await OrderController.handlePaymentAuthorized(webhook_payload, is_valid);
				break;
			}
			case 'payment.failed': {
	...
	private static async handlePaymentFailed(
		...
		if (is_valid) {
			const order_id = webhook_payload.payload.payment.entity.notes.platform_order_id;
